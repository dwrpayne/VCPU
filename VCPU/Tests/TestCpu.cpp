...
bool TestOpcodeDecoder(Verbosity verbosity)
{
		...
		{ A_SHRA, OP_SRAV, F_SRAV } }))
	{
		...
		success &= TestState(i++, (op == OP_SLL || op == OP_SRL || op == OP_SRA || op == OP_SLLV || op == OP_SRLV || op == OP_SRAV), test.ShiftOp().On(), verbosity);
		success &= TestState(i++, (op == OP_SLL || op == OP_SRL || op == OP_SRA), test.ShiftAmtOp().On(), verbosity);
	}
